...
Ukážte, že pre náhodné premenné, pre ktoré platí $Y = aX + b,$ je $\text{Corr}[X,Y] = 1$ alebo $\text{Corr}[X,Y] = -1.$
...
$$\text{Cov}[X,Y] = \text{E}[(X-\text{E}[X])(Y-\text{E}[Y])] = \text{E}[(X-\text{E}[X])(aX+b-\text{E}[aX+b])]=$$
$$\text{E}[(X-\text{E}[X])(aX-a\text{E}[X])] = \text{E}[(X-\text{E}[X])a(X-\text{E}[X])]=$$
$$a\text{E}[(X-\text{E}[X])(X-\text{E}[X])]=a\text{Var}[X]$$
...
$$\text{Corr}[X,Y] = \frac{\text{Cov}[X,Y]}{\text{sd}[X]\text{sd}[Y]} = \frac{a\text{Var}[X]}{\text{sd}[X]\text{sd}[Y]} = \frac{a}{|a|} = \begin{cases}
1, & \text{ak } a > 0 \\
-1, & \text{ak } a < 0
\end{cases}.$$
...
$$\text{E}[XY] = \int_{-\infty}^{\infty}\int_{-\infty}^{\infty} xy \, f_{XY}(x,y) \, dx \,dy= \int_{-\infty}^{\infty}\int_{-\infty}^{\infty} xy \, f_{X}(x) f_{Y}(y) \, dx \,dy = $$
$$\int_{-\infty}^{\infty} x \, f_{X}(x)  \, dx \int_{-\infty}^{\infty}y \,f_{Y}(y) \,dy =  \text{E}[X] \cdot \text{E}[Y]. $$
pre diskrétne rozdelené náhodné premenné by bol postup analogický.
...
Potom $\text{E}[X]=0, \text{E}[Y]=\frac{(1-(-1))^2}{12}=\frac{1}{3}, \text{E}[XY] = \text{E}[X^3] = 0,$
lebo $\text{E}[X^3]$ je intergrálom nepárnej funkcie na symetrickom intervale.)
